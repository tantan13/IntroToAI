...
def cornersHeuristic(state, problem):
    ...
    while len(unvisited) is not 0:
        nearestPos = unvisited[0]
        currD = util.manhattanDistance(currentPosition, unvisited[0])
        for i in range(len(unvisited) - 1):
            if util.manhattanDistance(currentPosition, unvisited[i+1]) < currD:
                nearestPos = unvisited[i+1]
                currD = util.manhattanDistance(currentPosition, unvisited[i+1])
        distLeft += currD
        unvisited.remove(nearestPos)
