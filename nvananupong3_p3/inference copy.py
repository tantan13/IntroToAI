...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # else: 
        # #else noisy distance == None
        # 	jailPosition = self.getJailPosition()
        # 	allPossible[jailPosition] = edgeCase;
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # 	newGhostPos = self.setGhostPosition(gameState, oldPos)
        #  	newPosDist = self.getPositionDistribution(newGhostPos)

        #  	for nextState in newPosDist.items():
        #  		allPossible[nextState[0]] = allPossible[nextState[0]] + allPossible[1] * self.beliefs[oldPos]
        # allPossible.normalize()
        # self.beliefs = allPossible
        ...
        for oldPos in self.legalPositions:
            newGhostPos = self.setGhostPosition(gameState, oldPos)
            newPos = self.getPositionDistribution(newGhostPos)
            for nextState in newPos.items():
                allPossible[nextState[0]] = allPossible[nextState[0]] + nextState[1] * self.beliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while i < numParticles:
            weight = legalPositions[i % len(legalPositions)]
            particlesList.append(weight)
            i += 1
    ...
    def observe(self, observation, gameState):
        ...
        for pos, val in particles.items():
            dist = util.manhattanDistance(pos, pacmanPosition)
            particles[pos] = val * emissionModel[dist]

        if particles.totalCount():
            self.particles = []
            for i in range(self.numParticles):
                self.particles.append(util.sample(particles))
        else:
            self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            update = util.sample(newPosDist)
            newParticles.append(update)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        particles = list(itertools.product(legal, repeat=self.numGhosts))
        ...
        for i, j in enumerate(self.particles):
            ...
            while ghost < self.numGhosts:
                if  noisyDistances[ghost] != None:
                    ...
                    prob = prob * emissionModels[ghost][dist]
                else:
                    j = self.getParticleWithGhostInJail(j, ghost)
                ghost += 1
            particles[j] += prob

        if particles.totalCount() == 0:
            self.initializeParticles()

        else:
            self.particles = [util.sample(particles) for i in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                newState = setGhostPositions(gameState, newParticle)
                newGhost = self.ghostAgents[i]
                newPosition = getPositionDistributionForGhost(newState, i, newGhost)
                newParticle[i] = util.sample(newPosition)
            "*** END YOUR CODE HERE ***"
