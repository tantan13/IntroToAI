...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghostPos in livingGhostPositionDistributions:
            bestPos = ghostPos.keys()[0]
            bestProb = ghostPos[bestPos]
            for positions, probability in ghostPos.items():
                if (probability > ghostPos[bestProb]):
                    bestProb = positions
                    bestPos = probability
            pos.append(bestProb)

        for p in pos:
            legalDist = self.distancer.getDistance(p, pacmanPosition)
            dist.append(legalDist)

        minD = pos[dist.index(min(dist))]

        for legalActions in legal:
            action = Actions.getSuccessor(pacmanPosition, legalActions)
            minDist = self.distancer.getDistance(minD, action)
            neighbors.append(minDist)

        shortestDist = float("inf")
        bestAction = None

        for neighbor in neighbors:
            if neighbor < shortestDist:
                shortestDist = neighbor
        bestIndex = neighbors.index(shortestDist)
        bestAction = legal[bestIndex]
