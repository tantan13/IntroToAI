...
class Perceptron(object):
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for i, w in enumerate(self.weights):
            ...
            newWeight = oldWeight + (alpha * inActs[i] * delta)
            weights.append(newWeight)
            update = abs(w - newWeight)
            totalModification += update
            w = newWeight
        inActs.pop(0)
        self.weights = weights
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            acts = list()
            for nodes in layer:
                acts.append(nodes.sigmoidActivation(curr))
            feed.append(acts)
            curr = acts
        return feed
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    for node in nextLayer:
                        update = node.weights[neuronNum + 1]
                        weight += deltas[0][index] * update
                        index += 1
                    delta = weight * gPrime
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    while weightMod > weightChangeThreshold and iteration < maxItr:
        error, weightMod = nnet.backPropLearning(examples = examplesTrain, alpha = alpha)
        iteration += 1
        # if iteration%10==0:
        #     print '! on iteration %d; training error %f and weight change %f'%(iteration,trainError,weightMod)
        # else:
    ...
    for example in examplesTest:
        ...
        last_output = output[len(output) - 1]
        test = []
        for i in last_output:
            test.append(int(round(i, 0)))
        if test == example[1]:
            testCorrect += 1
        else:
            testError += 1
